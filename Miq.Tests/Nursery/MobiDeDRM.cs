...
namespace Miq.Tests.Nursery.MobiDeDRM
{
    public class MobiReader
    {
        ...
        public String StringMetaData(uint metadataCode)
        {
            if (_Metadata == null)
            {
                ...
                if ((firstSection.ExthSectionOptions & 0x40) != 0 && endOffset - exthOffset >= 12)
                {
    ...
    public class Section
    {
        ...
        public UInt16 ExtraDataOptions
        {
            get
            {
                UInt16 flags = 0;
        ...
        public UInt32 ExthSectionOptions { get { return UInt32At(0x80); } }
    ...
    [TestClass]
    public class MobiDeDRMTests
    {
        [TestMethod]
        public void NotEncryptedBook()
        {
            var aMobiFile = "C:\\Users\\Miguel\\Documents\\My Kindle Content\\B000JML1QG_EBOK.azw";
            if (!File.Exists(aMobiFile))
            {
                Assert.Inconclusive("Didn't find the testing mobi file.");
                return;
            }

            MobiReader grimmStories = new MobiReader(aMobiFile);
            ...
            Assert.AreEqual(2, firstSection.ExtraDataOptions);
            Assert.AreEqual(80u, firstSection.ExthSectionOptions);
        ...
        {
            var aMobiFile = "C:\\Users\\Miguel\\Documents\\My Kindle Content\\B000FBFNII_EBOK.azw";
            if (!File.Exists(aMobiFile))
            {
                Assert.Inconclusive();
                return;
            ...
            MobiReader book = new MobiReader(aMobiFile);
    ...
    public class SectionHeaderItem : IRead
    {
        ...
        public void Read(BinaryReader reader)
        {
            ...
            Options = GenericReader.ReadByte(reader);
    ...
    public class PalmDocTests
    {
        ...
        public void foo()
        {
            var kindleDirectory = @"C:\Users\Miguel\Documents\My Kindle Content";
            if (!Directory.Exists(kindleDirectory))
            {
                Assert.Inconclusive("kindle directory not found");
                return;
            }

            foreach (var path in Directory.EnumerateFiles(kindleDirectory, "*.azw"))
            {
